...
A first issue is to determine the light threshold which should be done in the environment in the intended used.
Since this variable as been defined in a macro, it is easy to alter the value based on user's needs. 

The light intensity is controlled by the DC of a PWM. Since human eye resolution is about 16ms,
it was needed to keep the period below this value. A first more trivial solution to this could be to increase
